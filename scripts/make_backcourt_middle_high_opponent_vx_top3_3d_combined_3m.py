...
BASE_MANIFEST_PATH = PROBE_DIR / "top_shot_3d_views/top_shot_trajectories_3d_manifest.json"
VELOCITY_MANIFEST_PATH = PROBE_DIR / "top_shot_3d_views/top_shot_trajectories_3d_opponent_velocity_5ms_manifest.json"
OUTPUT_PATH = (
    PROBE_DIR
    / "backcourt_middle_high/opponent_midcourt_middle/"
    / "backcourt_middle_high__opponent_midcourt_middle__opponent_v_x_5ms_top3_shot_trajectories_3d_combined.png"
)

PANEL_IDS = [
    "backcourt_middle_high__opponent_midcourt_middle",
    "backcourt_middle_high__opponent_midcourt_middle__opponent_v_neg_x_5ms",
    "backcourt_middle_high__opponent_midcourt_middle__opponent_v_pos_x_5ms",
]
...
def main() -> None:
    ...
    from scripts.plot_controlled_contact_top_shot_trajectories_3d import (
        ...
        _expand_scenarios_over_opponent_velocity,
    )
    ...
    velocity_manifest = json.loads(VELOCITY_MANIFEST_PATH.read_text(encoding="utf-8"))

    grid_scenarios = _expand_scenarios_over_opponent_recovery_grid(
        list(probe_state["scenarios"]),
        "right",
        config,
    )
    velocity_scenarios = _expand_scenarios_over_opponent_velocity(
        [scenario for scenario in grid_scenarios if str(scenario["probe_id"]) == PANEL_IDS[0]],
        directions=("neg_x", "pos_x"),
        speed=5.0,
    )
    scenario_by_id = {
        str(scenario["probe_id"]): scenario
        for scenario in [*grid_scenarios, *velocity_scenarios]
    }
    top_shots_by_id = {
        PANEL_IDS[0]: base_manifest["top_shots"][PANEL_IDS[0]],
        PANEL_IDS[1]: velocity_manifest["top_shots"][PANEL_IDS[1]],
        PANEL_IDS[2]: velocity_manifest["top_shots"][PANEL_IDS[2]],
    }
    ...
    panels = [
        {
            ...
            "top_shots": top_shots_by_id[panel_id],
        }
    ...
    for panel, ax_pos, cbar_pos in zip(panels, template._panel_positions(), template._colorbar_positions()):
        ...
        template._draw_opponent_velocity_arrow(ax, scenario)
    ...
    handles = [
        ...
        Line2D([0], [0], color="#2563eb", lw=1.8, marker=">", markersize=6, label="opponent speed"),
    ]
